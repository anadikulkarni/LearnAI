          ...
          <div v-for="course in courses" :key="course.id" class="col-sm-6 col-lg-4 mb-4">
            <div class="block-4 text-center border">
              <div :id="course.id" :name="course.id">
                ...
                <div v-if="course.ratings[0].rating == 0"  class="input-group mb-3" style="max-width: 120px; text-align: center; margin-left: auto; margin-right: auto;" align="center">
                <h6 class="text-danger">NOT RECOMMENDED!</h6>
                <p class="text-danger mb-0">extremelylow rating!</p>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  computed: {
    // Computed property to get the category_id from the route query
    categoryId() {
  ...
  watch: {
    // Watch for changes in the `categoryId` computed property
    categoryId(newValue, oldValue) {
      // Perform an action when the category_id changes
      
      // Fetch or update data based on the new category_id
      this.getcourses()
      this.reloadPage()
    }
  },
  mounted() {
    if(this.$route.query.category_id) {
      ...
      this.endpoint = "courses/category/" + this.categoryid
    } else if (this.$route.query.keyword) {
      this.keyword = this.$route.query.keyword
      this.endpoint = "courses/search/" + this.keyword
    }
