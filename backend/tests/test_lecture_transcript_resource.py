...
#         lecture_transcript=LectureTranscript(content="Q3zLNZAZWxM1FPMd",course_id=1)
#         db.session.add(lecture_transcript)
#         db.session.commit()
#         start_time = time.time()
#         response = self.client.get(f'/api/lecture_transcript/{lecture_transcript.content}')
#         assert response.status_code == 200
#         assert isinstance(response.get_json(), list)
#         self.log_test(f'/api/lecture_transcript/{lecture_transcript.content}', response, start_time)
